...
(defun rand (dimensions)
  "Makes an array of shape DIMENSIONS and type T, filled with random numbers
   ...
   "
  (let* ((arr (make-array dimensions))
         (size (array-total-size arr)))
    (dotimes (i size)
      (setf (row-major-aref arr i) (random 1.0)))
    arr))
  ...
  (let* ((arr (make-array dimensions))
    ...
    (do ((i 0 (+ 2 i)))
      ...
      (let* ((u1 (random 1.0))
             ...
             (r (sqrt (* -2 (log u1)))))
        (setf (row-major-aref arr i) (* r (cos 2piu2)))
        (setf (row-major-aref arr (1+ i)) (* r (sin 2piu2)))))
    ;; If size is odd then one extra random number is needed
    (if (not (zerop (logand size 1)))
        (let* ((u1 (random 1.0))
               (2piu2 (* 2 pi (random 1.0)))
               (r (sqrt (* -2 (log u1)))))
          (setf (row-major-aref arr (1- size)) (* r (cos 2piu2)) )))
    arr))
